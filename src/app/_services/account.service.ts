...
export class AccountService {
      ...
      new Promise<any>(resolve =>
        FB.ui(
          ...
          function(response) {
            ...
            // && response.post_id
            if (response) {
              alert("Post was published. You are granted a discount!");
            } else {
              alert("Post was not published. No discount for you!");
            }
          }
        )
      )
  ...
  apiAuthenticate(accessToken: string) {
    ...
    return this.http.post<any>(`${baseUrl}/authenticate`, { accessToken }).pipe(
      map(account => {
